...
async def ai_product_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    config = load_config()
    pdf_path = Path(config.files.ai_product)
    df = read_curriculum_pdf(pdf_path)

    if df.empty:
        await update.message.reply_text("Не удалось извлечь данные из учебного плана AI Product.")
        return

    sem_counts = df.groupby("semester")["discipline"].count()
    message = "Учебный план программы 'AI Product':\n"
